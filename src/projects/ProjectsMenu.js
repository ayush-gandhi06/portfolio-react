...
export default class ProjectsMenu extends Component {
  ...
  scrollToSectionIndex(container, sections, index, behavior) {
    ...
    const top = Math.max(0, Math.min(maxScroll, targetTop));
    container.scrollTo({ top, behavior });
  }

  handleScroll = () => {
    const { isMobile } = this.state;
    const container = !isMobile && this.desktopScrollRef.current ? this.desktopScrollRef.current : null;
    ...
    const sectionIdx = this.getClosestSectionIndex(container);
    const activePanelIndex = sectionIdx <= 0 ? null : sectionIdx - 1;
    this.setState({ scrollProgress: progress, activePanelIndex });
  };

  scrollToPanel = (index) => {
    const container = this.desktopScrollRef.current;
    if (!container || this.state.isMobile) return;
    const sections = this.getDesktopSections();
    ...
    this.scrollToSectionIndex(container, sections, targetIndex, reduced ? "auto" : "smooth");
  };
